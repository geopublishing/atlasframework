...
import org.geopublishing.atlasStyler.swing.RasterRulesList_Intervals_GUI;
...
public class ImportWizardResultProducer_FILE extends ImportWizardResultProducer implements
        WizardResultProducer {

    final static Logger log = Logger.getLogger(ImportWizardResultProducer_FILE.class);
    ...
    public Object finish(Map wizardData) throws WizardException {
        ...
        final String selectedFilePath = (String) wizardData.get(ImportWizard.IMPORT_FILE);

        final AtlasStylerGUI asg = (AtlasStylerGUI) wizardData.get(ImportWizard.ATLAS_STYLER_GUI);
        ...
        DeferredWizardResult result = new DeferredWizardResult(true) {
            ...
            public void start(Map wizardData, ResultProgressHandle progress) {
                ...
                try {
                    ...
                    final String fileExtension = IOUtil.getFileExt(importFile, false).toLowerCase();

                    if (Arrays.asList(ASUtil.FILTER_SHAPE.getExtensions()).contains(fileExtension)) {
                        // Import Shape
                        summary = importShape(asg, importFile, progress, startTime);
                    } else if (Arrays.asList(ASUtil.FILTER_RASTERSUPPORTED.getExtensions())
                            .contains(fileExtension)) {
                        summary = importRaster(asg, importFile, progress, startTime);
                    }
            ...
            private Summary importRaster(final AtlasStylerGUI asg, File importFile,
                    ResultProgressHandle progress, long startTime) throws DataSourceException {

                GeoTiffReader reader = new GeoTiffReader(IOUtil.fileToURL(importFile));

                final StyledGridCoverageReader styledReader = new StyledGridCoverageReader(reader,
                        importFile.toString(), importFile.getName(), null);

                File sldFile = IOUtil.changeFileExt(importFile, "sld");

                File importedSld = setSldFileAndAskImportIfExists(asg,
                        IOUtil.changeFileExt(importFile, "sld").getName(), styledReader, sldFile);
                ...
                // i8n
                JPanel sum = new JPanel(new MigLayout("wrap 1", "[grow]"));
                sum.add(new JScrollPane(new JLabel("Sucessfully imported a raster with "
                        + styledReader.getBandCount() + " band(s).")));
//                sum.add(new JButton(new AbstractAction() {
//
//                    @Override
//                    public void actionPerformed(ActionEvent e) {
//                        StylerDialog stylerDialog = AVDialogManager.dm_Styler.getInstanceFor(
//                                styledReader.getStyle(), asg.getStylerMapView().getLayerManager(),styledReader.getStyle());
//                    }
//                }));

                return Summary.create(sum, "ok");
            }

            /**
             * Import a Shapefile selected by the Wizard into the AtlasStylerGUI
             */
            private Summary importShape(final AtlasStylerGUI asg, File importFile,
                    ResultProgressHandle progress, long startTime) throws FileNotFoundException,
                    IOException {
                URL urlToShape;
                 ...
                 * file
                 */
                if (!IOUtil.canWriteOrCreate(qixFile) || !IOUtil.canWriteOrCreate(fixFile)) {
                ...
                if (dataStore == null)
                    throw new RuntimeException("Could not read as ShapefileDataStore: "
                ...
                if (prjFile.exists()) {
                    try {
                        ...
                        prjCRS = null;
                        if (!AVSwingUtil.askOKCancel(asg, ASUtil.R(
                                "AtlasStylerGUI.importShapePrjBrokenWillCreateDefaultFor", e
                                        .getMessage(), prjFile.getName(), GeoImportUtil
                    ...
                    if (!AVSwingUtil.askOKCancel(asg, ASUtil.R(
                            ...
                            prjFile.getName(), GeoImportUtil.getDefaultCRS().getName()))) {
                        dataStore.dispose();
                        abort();
                        return null;
                    }
                ...
                File importedSld = setSldFileAndAskImportIfExists(asg,
                        IOUtil.changeFileExt(importFile, "sld").getName(), styledFS, sldFile);
                ...
                return Summary.create(
                        new JScrollPane(getSummaryPanelShapefile(startTime, countFeatures,
                                styledFS, importedSld)), "ok");
