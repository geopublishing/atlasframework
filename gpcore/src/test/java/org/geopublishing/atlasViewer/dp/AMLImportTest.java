...
public class AMLImportTest extends TestingClass {
    ...
    public void testImportExportBandNames() throws Exception {
        AtlasConfigEditable ace = GpTestingUtil.getAtlasConfigE(TestAtlas.rasters);
        Collection<DpLayerRaster> rasterLayers = ace.getDataPool().getRasterLayers();
        for (DpLayerRaster dplr : rasterLayers) {
            dplr.setBandNames(new Translation("a"));
        }
        AtlasConfigEditable ace2 = GpTestingUtil.saveAndLoad(ace);
        Set<DpLayerRaster> rasterLayers2 = ace2.getDataPool().getRasterLayers();
        for (DpLayerRaster dplr2 : rasterLayers2) {
            for (Translation t : dplr2.getBandNames()) {
