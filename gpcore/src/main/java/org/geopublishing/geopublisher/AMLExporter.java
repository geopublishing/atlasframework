...
public class AMLExporter {
    ...
    public File writeBuildxml(File targetDir, String atlasBaseName) {
        ...
        if (buildFile.exists()) {
            ...
            if (!buildFile.delete()) {
                LOGGER.error("Could not delete exitsing " + buildFile.getAbsolutePath()
                        + ", to update it.");
            }
        }
    ...
    public boolean saveAtlasConfigEditable(final AtlasStatusDialogInterface statusWindow)
        ...
        try {
             ...
             */
            GeoExportUtil.writeProjectionFilePrefereEPSG(GeoImportUtil.getDefaultCRS(), new File(
                    getAce().getAd(), AtlasConfig.DEFAULTCRS_FILENAME));
            ...
            if (atlasXmlhasBeenBackupped)
                try {
                    ...
                    IOUtil.copyFile(LOGGER, new File(getAce().getAd(),
                            AtlasConfig.ATLAS_XML_FILENAME + ".bak"), getAtlasXml(), false);
                } catch (final IOException ioEx) {
                    LOGGER.error("error copying " + AtlasConfig.ATLAS_XML_FILENAME + ".bak back.",
                            ioEx);
                    throw (ioEx);
                }
    ...
    private final Document exportAtlasConfig() throws Exception {
        ...
        final Element supportedLanguages = document.createElementNS(AMLUtil.AMLURI,
                "supportedLanguages");
            ...
            if (de instanceof DpLayer) {
                ...
                try {
                    ...
                    StylingUtil.saveStyleToSld(
                            style,
                            DataUtilities.urlToFile(DataUtilities.changeUrlExt(
    ...
    protected Node exportFonts(Document document) {
        ...
        for (File f : listFiles) {
            ...
            try {
                ...
                LOGGER.info("Not exporting a reference to broken font " + relPath + " in "
                        + fontsDir, e);
            }
     ...
     */
    private Node exportMapPool(final Document document) throws DOMException, AtlasExportException,
    ...
    private Node exportMap(final Document document, final Map map) throws AtlasExportException {
        ...
        if (map.getDefaultMapArea() != null) {
            final Element startViewEnvelope = document.createElementNS(AMLUtil.AMLURI,
                    "startViewEnvelope");
            final Envelope area = map.getDefaultMapArea();
            startViewEnvelope.appendChild(document.createTextNode(area.getMinX() + ","
                    + area.getMinY() + " " + area.getMaxX() + "," + area.getMaxY()));
        ...
        if (map.getMaxExtend() != null) {
            ...
            maxExtend.appendChild(document.createTextNode(area.getMinX() + "," + area.getMinY()
                    + " " + area.getMaxX() + "," + area.getMaxY()));
            element.appendChild(maxExtend);
        }

        // Is the ScalePanel visible in this map?
        element.setAttribute(AMLUtil.ATT_MAP_SCALE_VISIBLE, Boolean.valueOf(map.isScaleVisible())
                .toString());
        ...
        element.setAttribute("gridPanelVisible", Boolean.valueOf(map.isGridPanelVisible())
                .toString());
        ...
        // Kartenrand)
        final File outputMapGridPanelPrjFile = new File(getAce().getHtmlDirFor(map),
        ...
        for (final String layerID : map.getAvailableCharts().keySet()) {
            ...
            final Element availableCharts = document.createElementNS(AMLUtil.AMLURI,
                    "availableCharts");
    ...
    private final Element exportTranslation(final Document document, final String tagname,
        ...
        if (translation == null) {
            ...
            for (final String key : translation.keySet()) {
                final Element descTranslation = document.createElementNS(AMLUtil.AMLURI,
                        "translation");
    ...
    private final Element exportDatapoolLayerVector(final Document document,
        ...
        if (filter != Filter.INCLUDE) {
            // Creating a aml:filename tag...
            final Element filterRuleElement = document
                    .createElementNS(AMLUtil.AMLURI, "filterRule");
        ...
        for (final FeatureChartStyle chartStyle : dpe.getCharts()) {
            ...
            try {
                ...
                if (chartStyle instanceof FeatureChartStyle) {
                    FeatureChartUtil.FEATURE_CHART_STYLE_FACTORY.writeStyleToFile(chartStyle,
                            "chartStyle", chartFile);
                } else {
                    ChartStyleUtil.CHART_STYLE_FACTORY.writeStyleToFile(chartStyle, "chartStyle",
                            chartFile);
                }
            } catch (final Exception e) {
    ...
    private Element exportAttributeMetadata(final DpLayerVectorFeatureSource dpe,
            final Document document, final AttributeMetadataImpl attrib) {
        ...
        while (children.hasMoreElements()) {
            ...
            if (nextElement instanceof Group) {
                ...
                throw new AtlasFatalException("Can't export Group " + group
                        + " because of an unknown TreeNode " + nextElement + " of class "
    ...
    private void copyAtlasMLSchemaFile() {
        try {
            ...
            org.apache.commons.io.FileUtils.copyURLToFile(resourceSchema, new File(getAtlasXml()
                    .getParentFile(), "AtlasML.xsd"));
        } catch (final Exception e) {
