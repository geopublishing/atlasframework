...
public class AtlasMenuBar extends JMenuBar {
    ...
    public AtlasMenuBar(AtlasViewerGUI atlasViewer) {
        ...
        if (!hasFileMenu) {
            ...
            AtlasJMenu fileMenu = new AtlasJMenu(GpCoreUtil.R("AtlasViewer.FileMenu"));
        ...
        if (!hasHelpMenu) {
            AtlasJMenu helpMenu = new AtlasJMenu(GpCoreUtil.R("AtlasViewer.HelpMenu"));
    ...
    private void group2Menus(JComponent parent, Group group) {
         ...
         */
        final int maxItems = new Double(Toolkit.getDefaultToolkit().getScreenSize().getHeight())
                .intValue() / 25;
        int countHere = 0;
        ...
        for (; children.hasMoreElements();) {
            ...
            if (countHere > maxItems) {
                JMenu weiterMenu = new JMenu(GpCoreUtil.R("AtlasViewer.MenuToLongForScreen.Next"));
    ...
    private void addFileMenuItems(JMenu fileMenu) {
         ...
         */
        JMenuItem screenshotMenuItem = new AtlasMenuItem(new AbstractAction(
                GpCoreUtil.R("AtlasViewer.FileMenu.JMenuItem.save_smart_screenshots"),
                Icons.ICON_SCREENSHOT_SMALL) {

            @Override
            public void actionPerformed(ActionEvent e) {

                if ((atlasViewer.getMap() == null) || (atlasViewer.getMapView() == null)) {
                    JOptionPane.showMessageDialog(atlasViewer.getJFrame(),
        ...
        }, GpCoreUtil.R("AtlasViewer.FileMenu.JMenuItem.ToolTip.save_smart_screenshots"));
        screenshotMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_PRINTSCREEN, 0, true));
        ...
        /**
         * The MenuItem to open the popup-window, if defined
         */
    ...
    private JMenuItem getPopupHTMLItem() {
        PopupHTMLItemAction popupHTMLItemAction = new PopupHTMLItemAction(atlasViewer);
     ...
     */
    public JMenuItem getJWSDownloadAllMenuItem() throws UnavailableServiceException {

        DownloadAllJNLPAction downloadAllJNLPAction = new DownloadAllJNLPAction(atlasViewer);

        JMenuItem jwsDownloadAllMenuItem = new AtlasMenuItem(downloadAllJNLPAction);

        ArrayList<String> haveToDownload = JNLPUtil.countPartsToDownload(atlasViewer
    ...
    private void addHelpMenuItems(JMenu helpMenu) {
        ...
        if (atlasViewer.getAtlasConfig().getAboutHTMLURL() != null) {
            ...
            aboutMenuItem.setText(GpCoreUtil.R("AtlasViewer.HelpMenu.About", atlasViewer
                    .getAtlasConfig().getTitle().toString()));
            aboutMenuItem.setToolTipText(GpCoreUtil.R("AtlasViewer.HelpMenu.About.tooltip",
    ...
    private JMenuItem getExitMenuItem() {
        ...
        exitMenuItem.setText(GpCoreUtil.R("AtlasViewer.FileMenu.ExitMenuItem.exit_application"));
    ...
    public void setJCheckBoxMenuItemAntiAliasing(JCheckBoxMenuItem jCheckBoxMenuItemAntiAliasing) {
    ...
    public JCheckBoxMenuItem getJCheckBoxMenuItemAntiAliasing() {
        if (jCheckBoxMenuItemAntiAliasing == null) {
            ...
            jCheckBoxMenuItemAntiAliasing.setSelected(atlasViewer.getAtlasConfig().getProperties()
