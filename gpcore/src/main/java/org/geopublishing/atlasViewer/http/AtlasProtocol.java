...
public enum AtlasProtocol {
     ...
     */
    public static boolean performLink(Component parent, AtlasConfig atlasConfig, String currURL,
	    String destURL) {
	try {
		...
		evDesc.replace("../../..", "../..");
		evDesc.replace("..\\..\\..", "..\\..");
		LOGGER.info("The URL to the PDF has been changed from " + before + " to " + evDesc
			+ " bacause of JWS.");
	    } else {
		...
		if (dpe != null && dpe instanceof DpMediaPDF) {
		    AtlasProtocol.PDF.performPDF(parent, AVSwingUtil.getUrl(dpe, parent), dpe
			    .getTitle().toString());
		} else {
		    /**
		    ...
		    pdfUrl = dpMediaOnlyUsedAsLinksUrlFix(pdfUrl, basePath, pdfPathOrName);
		    AtlasProtocol.PDF.performPDF(parent, pdfUrl, new File(pdfPathOrName).getName());
		}
		...
		AtlasProtocol.MAP.performMap(parent, AtlasProtocol.MAP.cutOff(destURL.toString()),
			atlasConfig);
		return true; // stop standard link processing
	    }
		...
		if (dpe != null && dpe instanceof DpMediaPICTURE) {
		    ...
		    String picturePathOrName = AtlasProtocol.PICTURE.cutOff(evDesc);

		    String pictureUrlString = basePath + "/" + picturePathOrName;

		    URL pictureUrl = new URL(pictureUrlString);

		    pictureUrl = dpMediaOnlyUsedAsLinksUrlFix(pictureUrl, basePath,
			    picturePathOrName);
		    AtlasProtocol.PICTURE.performPicture(parent, pictureUrl, atlasConfig);
	    ...
	    ExceptionDialog.show(parent, e, null, "Error while handling a URL protocoll");
	}
     ...
     */
    private static URL dpMediaOnlyUsedAsLinksUrlFix(URL mediaUrl, String basePath,
	    String mediaPathOrName) throws MalformedURLException {
	URL url = mediaUrl;
     ...
     */
    public void performHtml(Component owner, URL url, String title, AtlasConfig atlasConfig) {
	final HTMLBrowserWindow browserWindow = new HTMLBrowserWindow(owner, url, title,
		atlasConfig);
     ...
     * @param atlasConfig
     */
    public void performMap(Component owner, String mapId, AtlasConfig atlasConfig) {
	...
	if (!AtlasViewerGUI.isRunning()) {
	    AVSwingUtil
		    .showMessageDialog(
			    owner,
			    GpCoreUtil.R("AtlasMapLink.NotOpeningBecauseNoAtlasViewerOpen",
				    map.getTitle()));
	    return;
	}
	...
	if (parentWindow instanceof HTMLBrowserWindow || parentWindow instanceof AtlasAboutDialog
