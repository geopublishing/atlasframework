...
public class ShpFiles {
    ...
    /**
     * Determine if the location of this shapefile is writable. Returns <code>true</code> if all the
     * potential Shape files (existing or not existing) could be written/created.
     * 
     * @return true if local, false if remote
     */
    public boolean isWriteable() {
        if (!isLocal())
            return false;
        for (ShpFileType sft : ShpFileType.values()) {
            URL url = urls.get(sft);
