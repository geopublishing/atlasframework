...
public abstract class DpEntry<CHART_STYLE_IMPL extends ChartStyle> implements
	 ...
	 * @throws AtlasFatalException
	 */
	public void exportTo(File targetDir, Component owner) throws IOException,
			AtlasFatalException {

		String ending = getFilename().substring(getFilename().lastIndexOf('.'));
		File targetFile = new File(targetDir, getTitle().toString().replace(
				' ', '_')
				+ ending);

		FileUtils.copyURLToFile(getUrl(owner), targetFile);
	...
	public URL getCharsetUrl() {
		return IOUtil.changeUrlExt(getUrl((Component)null), "cpg");
	}
