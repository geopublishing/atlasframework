...
rem  Diese Bibliothek ist freie Software; Sie duerfen sie unter den Bedingungen der GNU Lesser General Public License, wie von der Free Software Foundation ver�ffentlicht, weiterverteilen und/oder modifizieren; entweder gem�� Version 2.1 der Lizenz oder (nach Ihrer Option) jeder sp�teren Version.
rem  Diese Bibliothek wird in der Hoffnung weiterverbreitet, dass sie nuetzlich sein wird, jedoch OHNE IRGENDEINE GARANTIE, auch ohne die implizierte Garantie der MARKTREIFE oder der VERWENDBARKEIT F�R EINEN BESTIMMTEN ZWECK. Mehr Details finden Sie in der GNU Lesser General Public License.
...
javaw -Xmx320m -Dfile.encoding=UTF8  -Djava.library.path=. -cp %HOMEPATH%\.Geopublishing:gpswinggui-${project.version}.jar org.geopublishing.geopublisher.swing.GeopublisherGUI
