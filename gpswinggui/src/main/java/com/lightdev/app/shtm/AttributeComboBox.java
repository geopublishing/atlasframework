...
/**
 * ComboBox to show and manipulate an attribute out of a given set of attribute
 * values.
 * 
...
class AttributeComboBox extends JComboBox implements AttributeComponent {
	 ...
	 * 
	 * @param items
	 *            the items to appear in the list of this component
	 * @param names
	 *            the attributes to associate with items (in the same order)
	 * @param key
	 *            the CSS attribute key this component represents
	 * @param htmlKey
	 *            the HTL attribute key this component represents
	 * 
	 * @see getValue
	 */
	...
	public boolean setValue(AttributeSet a) {
		...
		if (attributeKey != null) {
			...
			/*
			 * correction start: missing list-style-type attribute from style
			 * sheet
			 */
			else if (valObj == null
				...
				if (name != null
						&& name.toString().equalsIgnoreCase(
			...
			/*
			 * correction end: missing list-style-type attribute from style
			 * sheet
			 */
			else {
				// System.out.println("AttributeComboBox setValue value=" +
				// valObj);
				setValue(valObj);
				success = true;
			}
	 ...
	 * 
	 * <p>
	 * If one an attribute key is not set, the value will not be returned for
	 * that attribute key. If both attribute keys are set, two attributes with
	 * identical value are returned. Up to J2SE 1.4, the Java language does not
	 * render CSS.Attribute.VERTICAL_ALIGN but it does render
	 * HTML.Attribute.VALIGN. Some browsers handle it vice versa, so it is
	 * useful to store both attributes.
	 * </p>
	 * 
	...
	public AttributeSet getValue() {
		...
		// System.out.println("AttributeComboBox getValue originalIndex=" +
		// originalIndex + " value=" + value);
		if (originalIndex != value) {
			// System.out.println("changed " + attributeKey + " originalIndex="
			// + originalIndex + " value=" + value);
			if (attributeKey != null) {
				Util.styleSheet().addCSSAttribute(a,
						(CSS.Attribute) attributeKey, names[value]);
