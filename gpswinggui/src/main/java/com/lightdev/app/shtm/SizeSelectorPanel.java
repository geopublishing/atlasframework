...
class SizeSelectorPanel extends JPanel implements AttributeComponent,
	 ...
	 * 
	 * @param key
	 *            the attribute key this instance of SizeSelectionPanel
	 *            represents
	 * @param allowNegative
	 *            true, if negative values are to be allowed in the panel, false
	 *            if not
	 */
	...
	/**
	 * construct a SizeSelectorPanel with a JSpinner to select a value and
	 * either a JComboBox to select a given unit for the selection value or a
	 * JLabel showing a fixed unit.
	 * 
	 * @param key
	 *            the attribute key this instance of SizeSelectionPanel
	 *            represents
	 ...
	 *            if not
	 * @param type
	 *            the type of unit indicator, one of TYPE_LABEL and TYPE_COMBO
	 */
	public SizeSelectorPanel(Object key, Object htmlKey, boolean allowNegative,
			int type) {
	...
	public void actionPerformed(ActionEvent ae) {
		if (ae.getSource().equals(unitSelector)) {
			// System.out.println("actionPerformed is unitSelector new value = "
			// + unitSelector.getSelectedItem().toString());
	...
	public void setValue(String val) {
		...
		// }
		/*
		 * else { newVal = Integer.parseInt(val); valueSelector.setValue(new
		 * Integer(val)); unit = UNIT_PT; }
		 */
		if (++setValueCalls < 2) {
	...
	public boolean setValue(AttributeSet a) {
		...
		// System.out.println("SizeSelectorPanel setValue SET attributeKey=" +
		// attributeKey + ", htmlAttrKey=" + htmlAttrKey);
		...
		if (valObj != null) {
			...
			if (htmlAttrKey != null) {
				...
				if (valObj != null) {
					// System.out.println("SizeSelectorPanel HTML valObj=" +
					// valObj);
					setValue(valObj.toString());
					success = true;
				}
	...
	/**
	 * adjust the minimum and maximum values of the component according to the
	 * unit
	 */
	private void adjustMinMax(String unit) {
		// if(lv != null) {
		SpinnerNumberModel model = (SpinnerNumberModel) valueSelector
				.getModel();
		int minVal = 0;
		if (allowNegative) {
