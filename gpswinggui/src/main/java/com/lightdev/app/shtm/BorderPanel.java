...
class BorderPanel extends JPanel implements AttributeComponent {
	...
	public BorderPanel() {
		...
		addSettings(g, c, Util.getResourceString("topLabel"),
				CombinedAttribute.ATTR_TOP, 0, 0);
		addSettings(g, c, Util.getResourceString("rightLabel"),
				CombinedAttribute.ATTR_RIGHT, 1, 1);
		addSettings(g, c, Util.getResourceString("bottomLabel"),
				CombinedAttribute.ATTR_BOTTOM, 1, 0);
		addSettings(g, c, Util.getResourceString("leftLabel"),
				CombinedAttribute.ATTR_LEFT, 0, 1);
	}
	...
	public AttributeSet getValue() {
		...
		Util.styleSheet().addCSSAttribute(set,
				CSS.Attribute.BORDER_RIGHT_WIDTH, newValue);
		Util.styleSheet().addCSSAttribute(set,
				CSS.Attribute.BORDER_BOTTOM_WIDTH, newValue);
	...
	public AttributeSet getValue(boolean includeUnchanged) {
		if (includeUnchanged) {
			...
			newValue = bWidth.getAttribute(CombinedAttribute.ATTR_TOP);
			Util.styleSheet().addCSSAttribute(set,
					CSS.Attribute.BORDER_TOP_WIDTH, newValue);
			Util.styleSheet().addCSSAttribute(set,
					CSS.Attribute.BORDER_RIGHT_WIDTH, newValue);
			Util.styleSheet().addCSSAttribute(set,
					CSS.Attribute.BORDER_BOTTOM_WIDTH, newValue);
			Util.styleSheet().addCSSAttribute(set,
					CSS.Attribute.BORDER_LEFT_WIDTH, newValue);
			return set;
		} else {
			return getValue();
		}
	...
	private class BorderSettings extends JPanel {
		 ...
		 * @param borderKey
		 *            the attribute key for the border width this object
		 *            represents
		 * @param colorKey
		 *            the attribute key for the border color this object
		 *            represents
		 */
		public BorderSettings(String title, int side) {
			...
			Util.addGridBagComponent(this, new JLabel(Util
					.getResourceString("borderWidthLabel")), g, c, 0, 0,
			...
			Util.addGridBagComponent(this, new JLabel(Util
					.getResourceString("borderColorLabel")), g, c, 0, 1,
					GridBagConstraints.EAST);
		 ...
		 * 
		 * @param color
		 *            the <code>CombinedAttribute</code> to take the color from
		 * 
		 */
