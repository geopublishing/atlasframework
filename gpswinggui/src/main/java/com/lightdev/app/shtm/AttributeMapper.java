...
/**
 * <p>
 * Maps HTML and CSS attributes to their equivalents to compensate discrepancies
 * in HTML and CSS rendering of various different view environments.
 * </p>
 * 
 * <p>
 * Introduced in stage 5 this class only contains hard wired fixes to certain
 * discrepancies. Should there come up an increased number of necessary fixes in
 * future stages, a more generic way of mapping (such as through a Hashtable of
 * from/to values), etc. will be done.
 * </p>
 * 
...
public class AttributeMapper extends SimpleAttributeSet {
	...
	private void mapToCSSAttributes() {
		...
		if (cssFontSize != null) {
			...
			addAttribute(CSS.Attribute.FONT_SIZE,
					SHTMLPanelImpl.FONT_SIZES[fontNumber - 1] + "pt");
		}
		mapToHTMLAttributes();
		...
		if (htmlFontFace != null) {
			if (cssFontFamily != null) {
				...
				if (cssFontFamily.toString().indexOf("Sans-Serif") > -1) {
					Util.styleSheet().addCSSAttribute(this,
							CSS.Attribute.FONT_FAMILY, "SansSerif");
				} else if (cssFontFamily.toString().indexOf("Monospace") > -1) {
					Util.styleSheet().addCSSAttribute(this,
							CSS.Attribute.FONT_FAMILY, "Monospaced");
				}
			} else {
				removeAttribute(HTML.Attribute.FACE);
				if (htmlFontFace.toString().indexOf("Sans-Serif") > -1) {
					...
					Util.styleSheet().addCSSAttribute(this,
							CSS.Attribute.FONT_FAMILY, htmlFontFace.toString());
				}
			}
		} else {
			if (cssFontFamily != null) {
