...
/**
 * An HTML editor. It is a browser-based component, which relies on the
 * FCKeditor, CKEditor or the TinyMCE editor.<br/>
...
public class JHTMLEditor extends NSPanelComponent {
	...
	public static class TinyMCEOptions {
		...
		/**
		 * Set custom HTML headers, which is mostly useful when integrating
		 * certain TinyMCE plugins.
		 */
		public static NSOption setCustomHTMLHeaders(
				final String customHTMLHeaders) {
		 ...
		 * The list of possible options to set for TinyMCE can be found here: <a
		 * href
		 * ="http://wiki.moxiecode.com/index.php/TinyMCE:Configuration">http:
		 * //wiki.moxiecode.com/index.php/TinyMCE:Configuration</a>.
		 * 
		 * @param optionMap
		 *            a map containing the key/value pairs accepted by TinyMCE.
		 * @return the option to set the options.
		 */
		public static NSOption setOptions(Map<String, String> optionMap) {
			final Map<String, String> optionMap_ = new HashMap<String, String>(
					optionMap);
	...
	public static class FCKEditorOptions {
		...
		/**
		 * Create an option to set custom Javascript configuration for the
		 * FCKeditor editor.<br/>
		 * The list of possible options to set for FCKeditor can be found here:
		 * <a href=
		 * "http://docs.fckeditor.net/FCKeditor_2.x/Developers_Guide/Configuration/Configuration_Options"
		 * >http://docs.fckeditor.net/FCKeditor_2.x/Developers_Guide/
		 * Configuration/Configuration_Options</a>.<br/>
		 * 
		 * @param javascriptConfiguration
		 *            the javascript configuration.
		 * @return the option to set a custom configuration.
		 */
		public static NSOption setCustomJavascriptConfiguration(
				final String javascriptConfiguration) {
	...
	public static class CKEditorOptions {
		 ...
		 * The list of possible options to set for CKEditor can be found here:
		 * <a href=
		 * "http://docs.cksource.com/ckeditor_api/symbols/CKEDITOR.config.html"
		 * >http
		 * ://docs.cksource.com/ckeditor_api/symbols/CKEDITOR.config.html</a>.
		 * 
		 * @param optionMap
	 ...
	 */
	public JHTMLEditor(HTMLEditorImplementation editorImplementation,
			NSOption... options) {
		if (editorImplementation == null) {
			throw new NullPointerException(
					"The editor implementation cannot be null!");
		}
		...
		default:
			throw new IllegalStateException(
					"A suitable HTML editor (FCKeditor, CKeditor, TinyMCE) distribution could not be found on the classpath!");
		}
		...
		webBrowser.navigate(WebServer.getDefaultWebServer()
				.getDynamicContentURL(JHTMLEditor.class.getName(),
						String.valueOf(instanceID), "index.html"));
	...
	/**
	 * Get the web browser that contains this component. The web browser should
	 * only be used to add listeners, for example to listen to window creation
	 * events.
	 * 
	...
	protected static WebServerContent getWebServerContent(
		...
		JHTMLEditor htmlEditor = (JHTMLEditor) ObjectRegistry.getInstance()
				.get(instanceID);
		if (htmlEditor == null) {
			return null;
		}
		...
		return htmlEditor.getWebServerContent(httpRequest, resourcePath_,
				instanceID);
	}

	/**
	 * Serve the HTTP content requested by the editor web page, which can be
	 * altered by subclasses. Note that altering the default content is
	 * generally not needed and is not recommended.
	 * 
	 * @return the content.
	 */
	protected WebServerContent getWebServerContent(HTTPRequest httpRequest,
			String resourcePath, final int instanceID) {
		return implementation.getWebServerContent(httpRequest, resourcePath,
				instanceID);
	}
	...
	public void setHTMLContent(String html) {
		html = JHTMLEditor
				.convertLinksFromLocal(html.replaceAll("[\r\n]", " "));
	...
	/**
	 * Indicate whether the editor is dirty, which means its content has changed
	 * since it was last set or the dirty state was cleared.
	 * 
	...
	private void setDirty(boolean isDirty) {
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == HTMLEditorListener.class) {
				if (ev == null) {
					ev = new HTMLEditorDirtyStateEvent(JHTMLEditor.this,
							isDirty);
				}
				((HTMLEditorListener) listeners[i + 1])
						.notifyDirtyStateChanged(ev);
			}
		}
	}
	...
	static String convertLinksToLocal(String html) {
		...
		Pattern p = Pattern.compile("=\\s*\"("
				+ WebServer.getDefaultWebServer().getURLPrefix()
				+ "/resource/)([^/]+)/([^\"]+)\"\\s");
		for (Matcher m; (m = p.matcher(html)).find();) {
			...
			try {
				resource = new File(Utils.decodeURL(Utils.decodeURL(codeBase)),
						resource).toURI().toURL().toExternalForm();
			} catch (MalformedURLException e) {
			}
			html = html.substring(0, m.start(1)) + resource
					+ html.substring(m.end(3));
		}
		return html;
	}
	...
	static String convertLinksFromLocal(String html) {
		...
		for (Matcher m; (m = p.matcher(html)).find();) {
			...
			resource = WebServer.getDefaultWebServer().getResourcePathURL(
					Utils.encodeURL(resourceFile.getParent()),
					resourceFile.getName());
			html = html.substring(0, m.start(1)) + resource
					+ html.substring(m.end(2));
		}
		return html;
	}
	 ...
	 * 
	 * @param listener
	 *            The HTML editor listener to add.
	 */
	 ...
	 * 
	 * @param listener
	 *            the HTML editor listener to remove.
	 */
