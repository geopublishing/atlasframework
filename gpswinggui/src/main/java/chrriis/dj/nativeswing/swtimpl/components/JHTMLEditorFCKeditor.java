...
class JHTMLEditorFCKeditor implements JHTMLEditorImplementation {
	...
	public JHTMLEditorFCKeditor(JHTMLEditor htmlEditor,
			Map<Object, Object> optionMap) {
		if (getClass().getResource(PACKAGE_PREFIX + "fckeditor.js") == null) {
			throw new IllegalStateException(
					"The FCKEditor distribution is missing from the classpath!");
		}
		this.htmlEditor = htmlEditor;
		customJavascriptConfiguration = (String) optionMap
				.get(JHTMLEditor.FCKEditorOptions.SET_CUSTOM_JAVASCRIPT_CONFIGURATION_OPTION_KEY);
		// We want potential dialogs to actually be dialogs.
		htmlEditor.getWebBrowser().addWebBrowserListener(
	...
	public WebServerContent getWebServerContent(final HTTPRequest httpRequest,
			...
			return new WebServerContent() {
				...
				public InputStream getInputStream() {
					String content = "<html>"
							...
							+ LS
							+ "      var sendCommand = "
							+ JWebBrowser.COMMAND_FUNCTION
							+ ";"
							+ LS
							+ "      var htmlContent;"
							+ LS
							+ "      var htmlDirtyTracker;"
							+ LS
							+ "      var isDirtyTrackingActive = true;"
							+ LS
							+ "      function JH_checkDirty() {"
							+ LS
							...
							+ "          try {"
							+ LS
							+ "            htmlContent = inst.GetHTML();"
							+ LS
							+ "          } catch(e) {"
							+ LS
							+ "          }"
							+ LS
							+ "          htmlDirtyTracker = setTimeout('JH_checkDirty()', 1000);"
							+ LS
							+ "        } else {"
							+ LS
							+ "          try {"
							+ LS
							+ "            var newHtmlContent = inst.GetHTML();"
							+ LS
							+ "            if(newHtmlContent != htmlContent) {"
							+ LS
							+ "              htmlContent = null;"
							+ LS
							+ "              htmlDirtyTracker = null;"
							+ LS
							+ "              sendCommand('[Chrriis]JH_setDirty');"
							+ LS
							+ "            } else {"
							+ LS
							...
							+ "        if(htmlDirtyTracker) {"
							+ LS
							+ "          clearTimeout(htmlDirtyTracker);"
							+ LS
							+ "        }"
							+ LS
							+ "        htmlContent = null;"
							+ LS
							+ "        if(isDirtyTrackingActive) {"
							+ LS
							+ "          htmlDirtyTracker = setTimeout('JH_checkDirty()', 1000);"
							+ LS
							+ "        }"
							...
							+ "      }"
							+ LS
							+ "      function JH_setDirtyTrackingActive(isActive) {"
							+ LS
							+ "        isDirtyTrackingActive = isActive;"
							+ LS
							+ "        JH_clearDirtyIndicator();"
							+ LS
							+ "      }"
							+ LS
							...
							+ LS
							+ "        sendCommand('[Chrriis]JH_setLoaded');"
							+ LS
							+ "      }"
							+ LS
							+ "    </script>"
		...
		if ("jhtml_save".equals(resourcePath)) {
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					String html = JHTMLEditor.convertLinksToLocal(httpRequest
							.getHTTPPostDataArray()[0].getHeaderMap().get(
							EDITOR_INSTANCE));
					HTMLEditorSaveEvent e = null;
					for (HTMLEditorListener listener : htmlEditor
		...
		if ("jhtml_sendData".equals(resourcePath)) {
			String data = httpRequest.getHTTPPostDataArray()[0].getHeaderMap()
					.get(EDITOR_INSTANCE);
			tempResult = data;
			return new WebServerContent() {
				@Override
			...
			return new WebServerContent() {
				...
				public InputStream getInputStream() {
					...
					org.apache.log4j.Logger.getLogger(
							JHTMLEditorFCKeditor.class).info(
							"Tried to upload " + resourcePath + " to where!?!");
					...
					String content = "<html>"
							...
							+ "    <script type=\"text/javascript\">"
							+ LS
							+ "      alert('upload to local system are not allowed...');"
							+ LS + "    </script>" + LS + "  </head>" + LS
							+ "  <body>" + LS + "    Upload successful." + LS
							+ "  </body>" + LS + "</html>" + LS;
					return getInputStream(content);
				}
			};
		}
		if ("editor/filemanager/connectors/php/connector.php"
				.equals(resourcePath)) {
			Map<String, String> queryParameterMap = httpRequest
					.getQueryParameterMap();
			String command = queryParameterMap.get("Command");
			String content = null;
			if ("GetFoldersAndFiles".equals(command)
					|| "GetFolders".equals(command)) {
				final String type = queryParameterMap.get("Type");
				String currentDir = queryParameterMap.get("CurrentFolder");
				File[] roots = File.listRoots();
				StringBuilder sb = new StringBuilder();
				sb.append("<?xml version=\"1.0\" encoding=\"utf-8\" ?>");
				sb.append("<Connector command=\"").append(command)
						.append("\" resourceType=\"").append(type)
						.append("\">");
				sb.append("<CurrentFolder path=\"")
						.append(currentDir)
						.append("\" url=\"")
						.append(WebServer.getDefaultWebServer()
								.getResourcePathURL(currentDir, ""))
						.append("\" />");
				// try {
				// sb.append("<CurrentFolder path=\"").append(currentDir).append("\" url=\"").append(new
				// File(currentDir).toURI().toURL()).append("\" />");
				// } catch (MalformedURLException e) {
				// }
				if (("GetFoldersAndFiles".equals(command) || "GetFolders"
						.equals(command))
						&& currentDir.equals("/")
						&& roots.length > 1) {
					sb.append("<Folders>");
					for (File file : roots) {
						sb.append("<Folder name=\"")
								.append(Utils.escapeXML(file.getAbsolutePath()))
								.append("\"/>");
					}
					sb.append("</Folders>");
				} else {
					if ("GetFoldersAndFiles".equals(command)
							|| "GetFolders".equals(command)) {
						sb.append("<Folders>");
						for (File file : new File(currentDir)
							...
							sb.append("<Folder name=\"")
									.append(Utils.escapeXML(file.getName()))
									.append("\"/>");
						}
						sb.append("</Folders>");
					}
	...
	public void clearDirtyIndicator() {
		htmlEditor.getWebBrowser().executeJavascript(
				"JH_clearDirtyIndicator();");
	}

	public void setDirtyTrackingActive(boolean isDirtyTrackingActive) {
		htmlEditor.getWebBrowser().executeJavascript(
				"JH_setDirtyTrackingActive(" + isDirtyTrackingActive + ");");
	}
	...
	public void setHTMLContent(String html) {
		...
		// "');");
		// There is a problem: IE crashes when it has the focus and is flooded
		// with this message.
		// While waiting for an SWT fix, we use the workaround to disable the
		// component which loses the focus.
		JWebBrowser webBrowser = htmlEditor.getWebBrowser();
